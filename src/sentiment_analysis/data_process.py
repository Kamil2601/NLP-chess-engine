...
full_regex = re.compile(
    "\d{1,}\.{1,3}\s?(([Oo0]-[Oo0](-[Oo0])?|[KQRBN]?[a-h]?[1-8]?[x-]?[a-h][1-8]?(\=[QRBN])?[+#]?[!?]*)(\s?\{.+?\})?(\s(1-0|0-1|1\/2-1\/2))?\s){1,2}"
)
...
def token_filter(token):
    return not re.search(punctuation_regex, token.lemma_) or re.match(
        notation_token, token.lemma_
    )
...
def remove_bad_tokens(preprocessed_comment):
    bad_tokens = set(["[", "]", "csl", "cal", "\\"] + list("bcdfghjklmnpqrstvwxyz"))
    return [token for token in preprocessed_comment if token not in bad_tokens]


def add_comments_with_removed_quality_marks(
    df_comments: pd.DataFrame, comment_col="comment"
):
    """
    ...
    df_comments_copy = df_comments[
        df_comments[comment_col].str.match("^[!?]{1,2}[ ]")
    ].copy()
    df_comments_copy[comment_col] = df_comments_copy[comment_col].map(
        remove_quality_mark
    )
...
def spacy_tokenize(comments, vocab, doc_to_tokens, split = 200000):
    nlp = spacy.load("en_core_web_sm", disable=["parser", "ner"])
    ...
    for i in range(0,len(comments), split):
        docs = [t for t in nlp.pipe(comments[i:i+split], n_process=16, batch_size=128)]
        tokenized_comments += [doc_to_tokens(doc, vocab) for doc in docs]

    return tokenized_comments


def nltk_tokenize(comments, vocab, doc_to_tokens):
    return [list(filter(lambda w: w in vocab, word_tokenize(com.lower()))) for com in comments]
...
def prepare_data_for_sentiment_analysis_training(
    df_moves: pd.DataFrame,
    vocab,
    comment_col="comment",
    sentiment_col="sentiment",
    doc_to_tokens=doc_to_text_or_lemma_tokens,
    min_len=2,
    max_len=100,
):
    # Filter moves, leave only with 0/1 sentiment
    df_comments = df_moves[df_moves[sentiment_col].isin([0, 1])][
        [comment_col, sentiment_col]
    ]
    ...
    # tokenize with Spacy
    df_comments["preprocessed_comment"] = spacy_tokenize(
        df_comments[comment_col], vocab, doc_to_tokens
    )

    df_comments["preprocessed_comment"] = df_comments["preprocessed_comment"].map(
        remove_bad_tokens
    )

    df_comments = df_comments[
        df_comments["preprocessed_comment"].map(len).between(min_len, max_len)
    ]
...
def prepare_data_for_sentiment_analysis_prediction(
    df_moves: pd.DataFrame,
    vocab,
    comment_col="comment",
    prep_comment_col="preprocessed_comment",
    doc_to_tokens=doc_to_text_or_lemma_tokens,
    min_len=2,
    max_len=100,
):
    ...
    # tokenize with Spacy
    df_moves[prep_comment_col] = spacy_tokenize(
        df_moves[prep_comment_col], vocab, doc_to_tokens
    )

    df_moves[prep_comment_col] = df_moves[prep_comment_col].map(remove_bad_tokens)
...
class DataProcessing:
    def __init__(
        ...
        min_len=2,
        max_len=100,
        preprocess_fun=remove_useless,
        postprocess_fun=remove_bad_tokens,
        tokenize_fun = spacy_tokenize
    ) -> None:
        self.vocab = vocab
        self.doc_to_tokens = doc_to_tokens
    ...
    def prepare_data_for_sentiment_analysis_training(
        ...
        if self.preprocess_fun:
            df_comments[prep_comment_col] = df_comments[comment_col].map(
                self.preprocess_fun
            )


        # tokenize with Spacy
        df_comments[prep_comment_col] = self.tokenize_fun(
            df_comments[prep_comment_col], self.vocab, self.doc_to_tokens
        )

        if self.postprocess_fun:
            df_comments[prep_comment_col] = df_comments[prep_comment_col].map(
                self.postprocess_fun
            )


        df_comments = df_comments[
            df_comments[prep_comment_col]
            .map(len)
            .between(self.min_len, self.max_len + 1)
        ]

        df_comments = shuffle_and_sort(df_comments, prep_comment_col)
    ...
    def prepare_data_for_sentiment_analysis_prediction(
        ...
        if self.preprocess_fun:
            df_moves[prep_comment_col] = df_moves[comment_col].map(self.preprocess_fun)

        # tokenize with Spacy
        df_moves[prep_comment_col] = self.tokenize_fun(
            df_moves[prep_comment_col], self.vocab, self.doc_to_tokens
        )

        if self.postprocess_fun:
            df_moves[prep_comment_col] = df_moves[prep_comment_col].map(
                self.postprocess_fun
            )

        df_moves = df_moves[
            df_moves[prep_comment_col].map(len).between(self.min_len, self.max_len + 1)
        ]
